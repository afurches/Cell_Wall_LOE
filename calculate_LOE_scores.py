...
# CALCULATE TOTAL BREADTH SCORES
print('Calculating total breadth scores.\n')
breadth_tot_dict = dict.fromkeys(gene_list,0)
for g in gene_list:
    for j in breadth_dict[g]:
        if breadth_dict[g][j] > 0:
            breadth_tot_dict[g] += 1

### START HERE ### ==> 
#Traceback (most recent call last):
#  File "calculate_LOE_scores.py", line 186, in <module>
#    breadth_tot_dict[k] += 1
#KeyError: 'metabolomics_1120_C1_165_7.07'

# CREATE LOE SCORES TABLE
print('Tallying final scores.\n')
            
# convert total breadth scores dictionary to pandas dataframe
br_df = pd.DataFrame.from_dict(breadth_tot_dict,orient='index',dtype=int)
#gene ids are index; one column of breadth counts
br_df.rename(columns={0:'breadth_tot'},inplace=True)

# convert depth scores dictionary to pandas dataframe
dep_df = pd.DataFrame.from_dict(depth_dict,orient='index',dtype=int)
dep_cols = dep_df.columns.tolist()

# concatenate depth scores dataframe onto breadth scores dataframe
LOE_scores = pd.concat([br_df,dep_df],axis=1,join='outer',ignore_index=False,sort=True)
LOE_scores['depth_tot'] = LOE_scores[[i for i in dep_cols]].sum(axis=1)
LOE_scores = LOE_scores.fillna(0.0).astype(int)
